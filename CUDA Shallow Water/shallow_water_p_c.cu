...
void vec_add(float* out_u, float* out_v, float* out_h, float* in_u, float* in_v, float* in_h) {
	...
	//Compute u & v at step t + dt: predictor step
	...
	//Use new u & v to compute h at time t + dt: full step for h


	adv_u_x = (1.0f / (2.0f * dx)) * (out_u[W] - out_u[E]);

	adv_u_y = (1.0f / (2.0f * dy)) * (out_u[N] - out_u[S]);

	adv_v_x = (1.0f / (2.0f * dx)) * (out_v[W] - out_v[E]);

	adv_v_y = (1.0f / (2.0f * dy)) * (out_v[N] - out_v[S]);


	float dh = 0.0f - (out_u[global_index] * adv_h_x) - (in_h[global_index] * adv_u_x) -
	...
	//Use new h at t + dt to compute u & v at t + dt: corrected velocity step



	adv_h_x = (1.0f / (2.0f * dx)) * (out_h[W] - out_h[E]);

	adv_h_y = (1.0f / (2.0f * dy)) * (out_h[N] - out_h[S]);

	adv_u_x = (1.0f / (2.0f * dx)) * (in_u[W] - in_u[E]);

	adv_u_y = (1.0f / (2.0f * dy)) * (in_u[N] - in_u[S]);

	adv_v_x = (1.0f / (2.0f * dx)) * (in_v[W] - in_v[E]);

	adv_v_y = (1.0f / (2.0f * dy)) * (in_v[N] - in_v[S]);


	du = 0.0f - (g * adv_h_x) - (b * in_u[global_index]) + (nu * laplacian_u) -
		(in_u[global_index] * adv_u_x + in_v[global_index] * adv_u_y);

	dv = 0.0f - (g * adv_h_y) - (b * in_v[global_index]) + (nu * laplacian_v) -
		(in_u[global_index] * adv_v_x + in_v[global_index] * adv_v_y);
